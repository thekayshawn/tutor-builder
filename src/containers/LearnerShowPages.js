...
class LearnerShowPages extends Component {
  ...
  async componentDidMount() {
    ...
    const result = await authAxios
      .get(`${api_hosting}/learning-material/fetch-pages/${id}?order=ASC`)
    ...
    const myresp = await axios
      .get(`${api_host}/learning-material/fetch-page-content/${page_id}`)
      .catch((err) => console.log(err));
  ...
  getPageContentData = async (page_number) => {
    const page_id =
      this.state.learning_material.length > 0
        ? this.state.learning_material[page_number].id
        : 0;
    const myresp = await axios
      .get(`${api_host}/learning-material/fetch-page-content/${page_id}`)
      .catch((err) => console.log(err));
  ...
  render() {
    var page_content =
      this.state.mydata.length > 0 ? this.state.mydata[0]?.html : "";

    page_content = page_content.replace(/checked="checked"/gi, "");
    let pageId =
      this.state.mydata.length > 0 ? this.state.mydata[0].page_id : "";
    localStorage.setItem("pageId", pageId);

    let questions =
      this.state.mydata.length > 0 ? this.state.mydata[0].questions : "";
                    ...
                    <Row key={index}>
                      <Col lg={4} md={4} sm={12}>
                        <div className="learner_side_img">
                          <img
                            src="assets/minimalist-blocks/preview/learnerside.svg"
                            alt="img-select"
                          />
                        </div>
                      </Col>
                      <Col lg={8} md={8} sm={12} className="learner-sieing">
                        <div className="learner_side_heading">
                          <button
                            onClick={this.pageContent.bind(this, item, index)}
                          >
                            <h2>{item.title}</h2>
                          </button>
                        </div>
    ...
    return (
      <>
        <div className="learner_main_header">
          <Container fluid>
            <Row>
              ...
              <Col xxl={4} xl={4} lg={4} md={6} sm={12}></Col>
              <Col xxl={4} xl={4} lg={4} md={6} sm={12}>
                <nav class="animated bounceInDown learner_signout">
                  <ul className="learner_list_item">
                    <li class="learner-sub-menu">
                      <a
                        className="learner_first_item"
                        href="javascript:void(0)"
                      >
                        <img
                          src="assets/minimalist-blocks/preview/icon/signout.svg"
                          alt="img-select"
                        />
                      ...
                      <ul>
                        ...
                        <li>
                          <Link to="/coursesavailble">SignOut</Link>
                        </li>
                      </ul>
                    </li>
                  </ul>
        ...
        <div className="learner-main-content-data"></div>
              ...
              <Col xl={12} lg={12} md={12} sm={12}>
                <div className="learner_main_content">
                  ...
                  <div
                    data-is-lerner="true"
                    data-page-id={pageId}
                    data-content-id={this.state.content_id}
                  >
                    {" "}
                    ...
                    <Col lg={12} md={12} sm={12}>
                      <div className="learner-advertises-heading">
                        <h5>
                          ...
                          <a href="javascript:void(0);">Vera White</a> Now!
                        </h5>
                      </div>
                    </Col>
                  </Row>
        ...
        <div className="learner-advertise">
          ...
          <div className="learner-advertise-heading">
            ...
            <button type="button" id="learner_close_advertis" class="close">
              &times;
            </button>
          </div>
        </div>
