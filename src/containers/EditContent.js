...
export default class Edit extends Component {
  ...
  handleOnSave = async (html, pageNumber) => {
    var subjective_questions = document.querySelectorAll(
      ".subjective_question"
    );
    ...
    const page_id =
      this.state.learning_material.length > 0
        ? this.state.learning_material[0].id
        : 0;
  ...
  async componentDidMount() {
    ...
    const myresp = await axios.get(
      `${api_host}/learning-material/fetch-page-content/${id}`
    );
    ...
    console.log("id1", id1);
    const result1 = await authAxios.get(
      `${api_hosting}/learning-material/fetch-page-detail/${id1}`
    );
    ...
    const result = await authAxios.get(
      `${api_hosting}/tutor/learning-material/fetch`
    );
  ...
  fetchThePagination = async (pageNumber) => {
    ...
    if (pageNumber == 1) {
      const id = this.props.match.params.id;
      const result = await authAxios.get(
        `${api_hosting}/tutor/learning-material/fetch/${id}`
      );
      console.log("Result", result);
      var number_of_pagies = result.data.number_of_pages
        ? result.data.number_of_pages + 1
...
else
{
  ...
  tutorCreate = async (e) => {
    ...
    const res = await authAxios.post(
      `${api_hosting}/learning-material/add-page`,
      formData,
      {
        headers: {
  ...
  pageDelete = async () => {
    ...
    if (this.state.number_of_pages == 1) {
      ...
      const updatedTasks = this.state.creates.find(
      ...
      axios
        .delete(`${api_host}/create/tutorPageDelete/${pageId}`)
        .then((res) => {
    ...
    student_HTMLTABLE =
      this.state.learning_material.length > 0
        ? this.state.learning_material?.map((item, index) => {
            return (
              <>
                <li key={index} className="setting-01">
                  <Link to="#">
