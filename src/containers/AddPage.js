...
function AddPage() {
  ...
  useEffect(async () => {
    ...
    const resp = await authAxios.get(
      `${api_hosting}/tutor/learning-material/fetch?page=1&limit=${limit}&ORDER=DESC`
    );
  ...
  const fetchPagination = async (currentPage) => {
    ...
    const res = await authAxios.get(
      `${api_hosting}/tutor/learning-material/fetch?page=${currentPage}&limit=${limit}&ORDER=DESC`
    );
