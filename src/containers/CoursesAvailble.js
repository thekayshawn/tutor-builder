...
function CoursesAvailble(props) {
  const [contentLearner, setContent] = useState({
    learning_material: [],
    isLoading: true,
  });
  ...
  useEffect(async () => {
    ...
    const resp = await authAxios.get(
      `${api_hosting_main}/learner/learning-material/list`
    );
  ...
  const fetchPagination = async (currentPage) => {
    ...
    const res = await authAxios.get(
      `${api_hosting_main}/learner/learning-material/list?page=${currentPage}&limit=${limit}&ORDER=DESC`
    );
  ...
  const handlePagePagination = async (data) => {
    ...
    setContent({
      ...contentLearner,
      learning_material: commentsFormServer,
      isLoading: false,
    });
  };
                ...
                <>
                  <Col xl={4} lg={4} md={4} sm={12} key={index}>
                    <div className="learner-courses">
                      ...
                      <h4>
                        <Link to={`learnershowpages/${item.id}`}>
                          {item.title}
                        </Link>
                      </h4>
                    </div>
